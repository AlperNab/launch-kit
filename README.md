...
**Domain:** `Startup / GTM` · **Suite:** `E-commerce Growth Suite` · **Accent:** `#a3e635`
...
**Core job:** Idea/product → launch assets.
...
|---|---:|:---:|---|---|
| `idea`<br>Idea | text | Yes | — | Affects input: Idea. |
...
| `product_type`<br>product type | text | No | — | Affects customization: product type. |
| `market`<br>market | text | No | — | Affects customization: market. |
| `price`<br>price | text | No | — | Affects customization: price. |
| `channel`<br>channel | text | No | — | Affects customization: channel. |
| `launch_date`<br>launch date | text | No | — | Affects customization: launch date. |
...
| `audience_sophistication`<br>audience sophistication | select | No | general | Affects customization: audience sophistication. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9130
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9130`. |
...
```bash
curl http://127.0.0.1:9130/api/health
```
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9130 uvicorn app.main:app --host 0.0.0.0 --port 9130
```
...
- `plugins/launch-kit.json` for plugin metadata.
